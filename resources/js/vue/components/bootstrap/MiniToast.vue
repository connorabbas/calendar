<template>
    <div class="toast-container position-fixed" :class="positionClasses">
        <div class="toast align-items-center" :class="classes" role="alert" aria-live="assertive" aria-atomic="true"
            :data-bs-delay="timeout" ref="toastRef">
            <div class="d-flex">
                <div class="toast-body">
                    <slot name="body"></slot>
                </div>
                <button type="button" class="btn-close me-2 m-auto" :class="closeBtnClasses" aria-label="Close"
                    @click="hide()"></button>
            </div>
        </div>
    </div>
